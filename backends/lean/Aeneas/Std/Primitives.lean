...
/-- The `Poll` type mirrors Rust's `std::task::Poll`.

    An `async fn` is lowered to a state machine whose `poll` function returns
    either `Poll.ready v` (the future has completed with value `v`) or
    `Poll.pending` (the future is not yet complete and must be polled again).

    Phase 1 async codegen (Direction A): each `async fn foo(args) -> T` produces
      `def foo_poll : FooState → Result (Poll T × FooState)`
    and an optional driver wrapper
      `def foo_run  : … → Result T` (polls until ready, fuel-guarded).

    This type is NOT yet generated by Aeneas — it is a forward declaration that
    will be used once Charon gains async/coroutine LLBC support.
    See FUTURE_DIRECTION.md and tests/src/async_basic.rs for the roadmap.
-/
